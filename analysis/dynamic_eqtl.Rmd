...
fixed_res <- lapply(fixed_resfs,read.csv)
mixed_resfs <- list.files("data/",pattern="mixedmodel",full.names = T)
mixed_res <- lapply(mixed_resfs,read.csv)
...
dat.beta <- data.frame(beta=c(fix.beta,mix.beta),
                       oracle=factor(rep(rep(c(0.02,0.03,0.04,0.05),each=500),2)),
                       model=rep(c("fixed_effect","mixed_effect"),each=2000))
...
When we only run the procedure once, the two-step model took 2.5 ms in median, while the mixed model took 8.0 ms.
...
dat.time <- data.frame(time=c(fix.time,mix.time),
                       oracle=factor(rep(rep(c(0.02,0.04,0.03, 0.05),each=500),2)),
                       model=rep(c("fixed_effect","mixed_effect"),each=2000))
ggplot(dat.time,aes(x=oracle,y=time,fill=model)) + geom_boxplot() + theme_bw() +
  ylab("Running time (unit: second)")
```

There would be a greater speedup if we repeat the procedure with one trait and multiple SNPs, our method scales much better than mixed effect model. At 1000 SNPs, which is the usual number of cisSNP around genes at 250Kb distance, our two-step model took 2 ms while `MixedModels` took 9 seconds.
