...
$$
\hat{\gamma} = \dfrac{\sum W_d \theta'_d G}{\sum W_d G^2}
\\
...
$$
var(\hat \gamma) = \dfrac{1}{\sum W_dG^2} = \dfrac{var(Y)}{\sum G^2 X_i^2}
$$
...
First of all, we need to show that the weighted average of $\hat{\theta_d}$ is the unbiased estimator of $\beta_x$ given that genotype has mean zero. 

Based on inverse variance weighted (IVW) meta-analysis or direct derivation, the weighted average of $\hat{\theta}$ is
...
From the interaction model, we can find the estimate for $\beta_x$ is:
...
In this case, the estimator of $\gamma$ is simply. Let us denote the interaction term $X\cdot G$ as $X'$ for convenience.
...
As for the standard error of $\hat \gamma$, it is obvious that they are the same.
