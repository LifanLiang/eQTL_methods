...
$$
\begin{align}
\mathcal L(\hat\beta_i;\pi,\delta,\beta,U,Z_i,\mu,\sigma,s_i) &= P(\hat\beta_i|\beta_i,s_i) \cdot P(\beta_i|\mu,\sigma,Z=1) \cdot (Z_i=1|\pi) + P(\hat\beta_i|\beta_i=0,s_i) \cdot P(Z_i=0|\pi) \\
&= w_i P(\hat\beta_i|\beta_i,s_i) P(\beta_i|\mu,\sigma,Z=1) + (1-w_i)P(\hat\beta_i|\beta_i=0,s_i,Z=0)
\end{align}
$$
...
$$
\begin{align}
Q(\mu^{(t+1)},\sigma^{(t+1)}| \mu^{(t)}, \sigma^{(t)}) = \underset{\mu,\sigma}{\arg\max} &E_{\bar\beta,Z}\sum_i \left[logP(\hat\beta_i|\bar\beta_i,s_i) + logP(\bar\beta_i|\mu, \sigma, Z_i) + logP(Z_i|\pi) \right] \\
\propto &E_{\bar\beta,Z}\sum_i logP(\bar\beta_i|\mu, \sigma, Z_i=1) \\
=&\sum_i w_i \cdot logP(\bar\beta_i|\mu, \sigma, Z_i=1) \\
\propto &\sum_i w_i \left(log(\sigma) - \dfrac{E_{\beta}[(\bar\beta-\mu)^2]}{2\sigma^2}   \right)
\end{align}
$$
...
$$
\begin{align}
\sigma^2 &= \dfrac{\sum_i w_i E_{\beta}[(\bar\beta-\mu)^2]}{\sum_i w_i} \\
&=\dfrac{\sum_i w_i E[\bar\beta^2]}{\sum_i w_i} - 2\mu\dfrac{E[\bar\beta]}{\sum_i w_i} + \mu^2 \\
&=\dfrac{\sum_i w_i E[\bar\beta^2]}{\sum_i w_i} - \mu^2 \\
&=\dfrac{\sum_i w_i (Var(\bar\beta)+E[\bar\beta]^2)}{\sum_i w_i} - \mu^2
\end{align}
$$
...
![Performance of parameter estimation of EM algorithm within simulation experiment.](docs/assets/mirage_sim_params.pdf)
...
```{r,echo=F}
params.alphasm <- readRDS("data/params_alpham.RDS")
params.alphasm$pi
```

* Genetic effects of the variants ($\mu$)

```{r,echo=F}
params.alphasm$mu
```

* Standard deviation of the genetic effects ($\sigma$)

```{r,echo=F}
params.alphasm$sigma
```

We only found 9 genes with these parameters. Among them is ABCG5. 
...
## Symmetric bimodal distribution for risk variants {.hidden .unlisted .unnumbered}
