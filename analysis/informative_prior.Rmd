...
susie.fit <- susie(sim.res$X, sim.res$y, prior_weights = sim.res$pi,
                   L=sum(sim.res$gamma), coverage = 0.9, standardize = T,
                   compute_univariate_zscore = T)
...
```{r}
susie.fit <- susie(sim.res$X, sim.res$y,L=sum(sim.res$gamma))
susie_plot(susie.fit, y='PIP')
```
...
dat.cor2 <- matrix(nrow=5,ncol=3)
colnames(dat.cor2) <- c("lasso","susie","susie.prior")
...
```{r}
boxplot(dat.mse, ylab="Mean squared error", col=2:4)
```
