...
The SNPs were evenly split into 2 groups. One group has 90% of SNPs with 0.02% chance of being causal, while the other group has 10% of SNPs with 1% chance of being causal. But the causal SNPs have the same effect size of. Let $Z_j$ be the group index for the $j_{th}$ SNP, $\gamma_j$ be whether the $j_{th}$ SNP is causal, and $X_ij$ be the genotype for the $i_{th}$ individual. The genetic effect for the $j_{th}$ SNP ($\beta_j$) is generated as follow:

$$
\gamma_j \sim Bernoulli(\pi_{Z_j})
\\
\beta_j|\gamma_j = 1 \sim \sigma_1
\\
\beta_j|\gamma_j = 0 \sim \sigma_0
$$

where $\pi_k$ is the prior of being causal for the $k_{th}$ group, $\sigma_1$ is the fixed point distribution concentrated on 0.2 when the SNP is being causal, and $\sigma_0$ is the Dirac delta function.

In this simulation, we set ($\pi_k$) as $\{0.02\%, 0.04\%,0.08\%,0.16\%,0.32\%\}$. For the whole region, 0.118% ($\approx 4$) of the SNPs are expected to be causal. We set $\sigma=0.2$ and $\sigma_e=1.0$. Then we generated gene expression Y as:
...
sim_group_pi <- function(X, pi=pi, sigma=0.5, sigma.e=1.0) {
...
```{r}
library(glmnet)
lasso.fit <- glmnet(sim.res$X, sim.res$y, lambda = 0.1)
...
```{r}
library(glmnet)
net.fit <- glmnet(sim.res$X, sim.res$y, lambda=0.2, alpha=0.5,
                  penalty.factor = 1 - 0.2*(sim.res$gamma))
...
We used 5-fold cross validation over the 1000 individuals. Evaluation metric in the test set is $R^2$. The two SuSIE models are one with priors and one without, respectively. Data Synthesis was repeated 20 times. Two different definitions of $R^2$ are computed. The first one is the squared correlation between $y$ in the test set and $\hat y$. The other is $1-\dfrac{\sum(y-\hat y)^2}{\sum(y-\bar y)^2}$, where $\bar y$ is the mean of $y$ in the training set.
...
for(i in 1:5) {
  ...
  lasso.fit <- glmnet(X.train, y.train,lambda = 0.1)
...
### Comparison based on squared correlation

```{r}
set.seed(1234)
N <- 20
inds <- sample(rep(1:5,times=200))
#sim.res$X <- apply(sim.res$X,2,as.numeric)
#sim.res$X <- scale(sim.res$X, center=T, scale=T)
dat.cor2 <- matrix(nrow=N,ncol=4)
colnames(dat.cor2) <- c("lasso","lasso.prior","susie","susie.prior")

dat.r2 <- matrix(nrow=N,ncol=4)
colnames(dat.r2) <- c("lasso","lasso.prior","susie","susie.prior")

dat.mse <- matrix(nrow=N,ncol=4)
colnames(dat.mse) <- c("lasso","lasso.prior","susie","susie.prior")
...
for(j in 1:N) {
  ...
  lasso.fit <- glmnet(X.train, y.train,lambda = 0.1)
  lasso.y <- predict(lasso.fit, X.test)
  dat.cor2[j,"lasso"] <- cor(y.test,lasso.y)**2
  dat.r2[j,"lasso"] <- calc.r2(y.test,lasso.y,mean(y.train))
  dat.mse[j,"lasso"] <- calc.mse(y.test,lasso.y)

  lasso.fit <- glmnet(X.train, y.train,lambda = 0.1,
                      penalty.factor = 1 - 0.2*(sim.res$gamma))
  lasso.y <- predict(lasso.fit, X.test)
  dat.cor2[j,"lasso.prior"] <- cor(y.test,lasso.y)**2
  dat.r2[j,"lasso.prior"] <- calc.r2(y.test,lasso.y,mean(y.train))
  dat.mse[j,"lasso.prior"] <- calc.mse(y.test,lasso.y)
  
  susie.fit <- susie(X.train, y.train)
  susie.y <- predict(susie.fit, X.test)
  dat.cor2[j,"susie"] <- cor(y.test,susie.y)**2
  dat.r2[j,"susie"] <- calc.r2(y.test,susie.y,mean(y.train))
  dat.mse[j,"susie"] <- calc.mse(y.test,susie.y)
  
  susie.fit <- susie(X.train, y.train, prior_weights = sim.res$pi)
  susie.y <- predict(susie.fit, X.test)
  dat.cor2[j,"susie.prior"] <- cor(y.test,susie.y)**2
  dat.r2[j,"susie.prior"] <- calc.r2(y.test,susie.y,mean(y.train))
  dat.mse[j,"susie.prior"] <- calc.mse(y.test,susie.y)
}
...
```{r}
plot(x=dat.cor2[,1], y=dat.cor2[,2], xlab="Lasso without prior", 
     ylab="Lasso with prior", main="Squared correlation")
lines(c(0,1),c(0,1),col="blue")
```

```{r}
plot(x=dat.cor2[,3], y=dat.cor2[,4], xlab="SuSIE without prior", 
     ylab="SuSIE with prior", main="Squared correlation")
lines(c(0,1),c(0,1),col="blue")
```

### Comparison based on PVE (or compared to the intercept model)
...
```{r}
plot(x=dat.r2[,1], y=dat.cor2[,2], xlab="Lasso without prior", 
     ylab="Lasso with prior", main="PVE")
lines(c(0,1),c(0,1),col="blue")
```

```{r}
plot(x=dat.r2[,3], y=dat.cor2[,4], xlab="SuSIE without prior", 
     ylab="SuSIE with prior", main="PVE")
lines(c(0,1),c(0,1),col="blue")
```
