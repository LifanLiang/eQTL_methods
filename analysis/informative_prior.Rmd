...
```{r}
library(glmnet)
lasso.fit <- glmnet(sim.res$X, sim.res$y,lambda = 0.3)
...
susie_plot(susie.fit, y='PIP', main="SuSIE with prior")
```
...
susie_plot(susie.fit, y='PIP',main="SuSIE without prior")
```

### Cross validation

We used 5-fold cross validation over the 100 individuals. Evaluation metric in the test set is $R^2$. The two SuSIE models is one with priors and one without. Note that performance varies a lot depending on the training-testing split. In this particular case, the PVE on full dataset is 3.4%. Two different definitions of $R^2$ are computed. The first one is the squared correlation between $y$ in the test set and $\hat y$. The other is $1-\dfrac{\sum(y-\hat y)^2}{\sum(y-\bar y)^2}$, where $\bar y$ is the mean of $y$ in the training set.
