...
PVE varies because the genetic component of $var(Y)$ depends on the effect size and the allele frequency of causal SNPs. Mostly, the total PVE was less than 10%.
...
When SuSIE was run without the grouped prior, results change a bit for SNPs in LD of the causal one.
...
We used 5-fold cross validation over the 100 individuals. Use mean square error in the test set as the evaluation metric. The two SuSIE models is one with priors and one without.

```{r}
inds <- sample(rep(1:5,times=20))
sim.res$X <- apply(sim.res$X,2,as.numeric)
dat.mse <- matrix(nrow=5,ncol=3)
colnames(dat.mse) <- c("lasso","susie","susie.prior")
