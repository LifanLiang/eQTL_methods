...
sim_group_pi <- function(X, pi=pi, sigma=0.3, sigma.e=1.0) {
...
We used 10-fold cross validation over the 100 individuals. Evaluation metric in the test set is $R^2$. The two SuSIE models is one with priors and one without.

```{r}
inds <- sample(rep(1:10,times=10))
sim.res$X <- apply(sim.res$X,2,as.numeric)
dat.r2 <- matrix(nrow=10,ncol=3)
...
boxplot(dat.r2, ylab="R2", col=2:4)
```
