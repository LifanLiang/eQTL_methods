...
est.par <- function(beta.hat, se, C, mu, sigma, pi.init, delta.init, niter=5) {
...
In ground truth, we set $\pi=\{0.8,0.3,0.05\}$ and $\delta=0.2$.
...
```{r,echo=F, message=F}
res <- est.par(beta.hat, se.var, C, mu, sigma, pi=c(0.1,0.1,0.1), delta=0.6, niter=10)

plot(1:length(res$lik.tract), res$lik.tract, type="l", ylab="marginal data likelihood", xlab="EM iterations")
points(1:length(res$lik.tract), res$lik.tract, pch=20)
```

```{r,echo=F}
plot(1:length(res$delta.trac), res$delta.trac, type="l", ylab="Proportion of risk genes", xlab="EM iterations", ylim=c(0,1))
points(1:length(res$delta.trac), res$delta.trac, pch=20)
```
...
* There are 100,000 samples with 1000 genes. Each genes has 100 variants.
* Proportion of risk genes: $\delta=0.2$. Hence there are 200 risk genes.
* Proportion of risk variants for each category: $\pi=\{0.8,0.3,0.05\}$ 
* LoF risk variants: $\beta \sim Normal(\mu=0.3, \sigma=0.2)$
* Missense risk variants: $\beta \sim Normal(\mu=0.2, \sigma=0.2)$
* Synonymous risk variants: $\beta \sim Normal(\mu=0.1, \sigma=0.2)$
...
par.est <- est.par(beta.hat, se.var, C,mu, sigma, pi.init = rep(0.1,3), delta.init = 0.5)
