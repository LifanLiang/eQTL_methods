...
# Model parameters as positive control (all methods wll have high power)
# N <- 1000 # number of genes
# J <- 100 # number of variants per gene
# delta <- 0.2 # percent of risk genes
# pi <- c(0.8, 0.3, 0.05) # percent of risk variants in each variant category
# mu <- c(0.3, 0.2, 0.1) # mean effect size
# sigma <- c(0.2, 0.2, 0.2) # SD of effect size
# n.samples <- 100000 # sample size of association study
...
Now we have two groups of genes. The first group contains 200 genes, 30% of which are risk genes. The second group has 800 genes, 5% of which are risk genes. 
...
ggplot(dat, aes(x=method,y=count,fill=true_positive)) +
  geom_bar(stat="identity", position = position_stack()) + 
  ylab(paste0("# of significant genes at PIP>",thres.pip*100,"%")) + 
  theme_bw() +
...
```{r, class.source="scroll-300"}
### Modify delta into a vector of probabilities
...
res <- est.par.gene_prior(beta.hat, se.var, C, mu, sigma, pi.init=rep(0.1,3), delta.init=rep(0.1,2), G=G, niter=20, tol=0.5)
```

```{r,echo=F}

matplot(res$delta.trac[1:17,], type="l", ylab="Gene priors", xlab="EM iteration", main="Categorical gene priors")
```

```{r}
res.uniform <- est.par(beta.hat, se.var, C, mu, sigma, pi.init=rep(0.1,3), delta.init=0.2, niter=20, tol=0.)
plot(res.uniform$delta.trac, type="l", ylab="Gene priors", xlab="EM iteration", main="Uniform gene priors")
```
...
for (i in 1:N) {
  mirage.pip.uniform[i] <- comp.gene(beta.hat[i,], se.var[i,], C[i,], res.uniform$pi, mu, sigma, delta=res.uniform$delta)$post.gene
  mirage.pip.genegroup[i] <- comp.gene(beta.hat[i,], se.var[i,], C[i,], res$pi, mu, sigma, delta=res$delta[G[i]])$post.gene
}


```

Compared to uniform gene priors, categorical gene priors have higher powers and also more false positives.
